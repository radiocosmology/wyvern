...
use wyvern::interpolate::{
    InterpolationPlan, Interpolator, interp_last_ax_complex, interp_last_ax_complex_weighted,
    ...
    (
        ...
        $(
            if $y_dtype.is_equiv_to(&dtype::<$pytype>($py)) {
                ...
                return Ok($func::<$floatty, P>($py, $plan, &y_in, y_out));
            }
        )+
    };
}
...
pub fn dispatch_unweighted<'py, P>(
    py: Python<'py>,
    plan: &P,
...
where
    P: InterpolationPlan + Interpolator<f32> + Interpolator<f64> + Sync,
{
